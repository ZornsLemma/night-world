...
constant S_OP_REMOVE = 2
  ...
  200DEFPROClee_sprite_reset:Y%=S_OP_REMOVE:FORn%=9TO12:W%=n%:CALLS%:NEXT:Y%=0:ENDPROC
  ...
  550NEXT:PROCreset_note_count:VDU19,1,colour1%;0;19,2,colour2%;0;19,3,colour3%;0;:PROCstop_sound:W%=SLOT_SUN_MOON:Y%=S_OP_REMOVE:CALLS%:PROClee_sprite_reset:K%=192
  551IFday_night%=0:day_night%=1:X%=25:W%=SLOT_SUN_MOON:CALLS%:CALLU%:PROCset_lee_sprite_from_lee_xy_os_gargoyle:full_speed_jump_time_limit%=45:max_jump_time%=90:PROChide_fleece:ENDPROC
  560full_speed_jump_time_limit%=20:max_jump_time%=40:day_night%=0:X%=24:W%=SLOT_SUN_MOON:CALLS%:CALLU%:PROCset_lee_sprite_from_lee_xy_os:PROCrestore_fleece:ENDPROC
 ...
 1160DEFPROCupdate_energy_and_items:IFX%=5:GOTO1210 ELSEIFX%=5OR(X%=7ANDlogical_room%<>1ANDlogical_room%<>5ANDlogical_room%<>9ANDlogical_room%<>14ANDlogical_room%<>7):GOTO1210
